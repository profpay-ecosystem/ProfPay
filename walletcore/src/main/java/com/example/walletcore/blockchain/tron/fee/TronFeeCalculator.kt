package com.example.walletcore.blockchain.tron.fee
...
class TronFeeCalculator(
    ...
    private val tronNodeStatusApi: TronNodeStatusApi,
    private val callService: TronCallApi,
) {
    val baseFee: BigInteger = BigInteger.valueOf(280_000)

    suspend fun calculate(params: ConfirmParams.TransferParams.Native, account: TronAccount?, accountUsage: TronAccountUsage?) =
        withContext(Dispatchers.IO) {
            val getParams = async {
                tronNodeStatusApi.getChainParameters().fold({ it.chainParameter }) { null }
            }
            ...
            val newAccountFeeInSmartContract =
                params?.firstOrNull { it.key == "getCreateNewAccountFeeInSystemContract" }?.value
    ...
    suspend fun calculate(params: ConfirmParams.TransferParams.Token, account: TronAccount?, accountUsage: TronAccountUsage?) =
        withContext(Dispatchers.IO) {
            val getParams = async {
                tronNodeStatusApi.getChainParameters().fold({ it.chainParameter }) { null }
            }
            ...
            val gasLimit = estimateTRC20Transfer(
                ...
                contractAddress = params.assetId.tokenId
                    ?: throw IllegalArgumentException("Incorrect contract on fee calculation"),
                value = params.amount,
            )
            ...
            val availableEnergy =
                BigInteger.valueOf(accountUsage?.energyLimit ?: 0L) - BigInteger.valueOf(
                    accountUsage?.energyUsed ?: 0L
                )
            val energyShortfall = BigInteger.ZERO.max(gasLimit.let {
                it.add(it.multiply(BigDecimal(0.2))).toBigInteger()
            } - availableEnergy)
    ...
    private suspend fun estimateTRC20Transfer(
        ...
        return BigDecimal.valueOf(result.energyUsed.toLong())
    }
}