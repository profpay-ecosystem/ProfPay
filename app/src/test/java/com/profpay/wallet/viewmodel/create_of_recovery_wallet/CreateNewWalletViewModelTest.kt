...
class CreateNewWalletViewModelTest {
    ...
    @Test
    fun `When initializing ViewModel, generateNewAddressAndMnemonic is called`() = runTest {
        coVerify { addressAndMnemonicRepo.generateNewAddressAndMnemonic() }
    }

    @Test
    fun `State is updated to Success when new data appears`() = runTest {
        val fakeResult = AddressGenerateResult(
