        ...
        composable(WalletSots.WalletAddress.route) {
            WalletAddressScreen(
                goToSendWalletAddress = { addressId: Long, tokenName: String ->
                    navController.navigate(
                        route =
                            WalletAddress.Send.createRoute(
                                addressId = addressId,
                                tokenName = tokenName,
                            ),
                    )
...
sealed class WalletInfo(
    ...
    object Send : WalletInfo(route = "send_wallet/{addressId}/{tokenName}") {
        fun createRoute(
            addressId: Long,
            tokenName: String,
        ): String = "send_wallet/$addressId/$tokenName"
    }
...
sealed class WalletAddress(
    ...
    object Send : WalletAddress(route = "send_from_wallet_address/{addressId}/{tokenName}") {
        fun createRoute(
            addressId: Long,
            tokenName: String,
        ): String = "send_from_wallet_address/$addressId/$tokenName"
    }
}

sealed class WalletSystem(
    val route: String,
) {
