...
fun UnifiedTransactionCard(
    ...
    val currentTokenName = TokenName.entries
        .find { it.tokenName == transaction.tokenName } ?: TokenName.USDT

    val currentAddress = if (transaction.receiverAddress == addressWa) {
    ...
    when (type) {
        TransactionCardType.INFO -> CardHistoryTransactionsFeature(
            onClick = {
        ...
        TransactionCardType.WA -> CardHistoryTransactionsForWAFeature(
            viewModel = viewModel!!,
            onClick = {
                sharedPref.edit { putLong("transaction_id", transaction.transactionId!!) }
                goToTXDetailsScreen()
        ...
        TransactionCardType.CA -> CACardHistoryTransactionsFeature(
            paintIconId = currentTokenName.paintIconId,
            shortNameToken = currentTokenName.shortName,
            transactionEntity = transaction.toEntity(),
            amount = decimalFormat(transaction.amount.toTokenAmount()),
